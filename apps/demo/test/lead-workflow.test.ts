...
describe("example-owned workflow with Attio adapter", () => {
  it.each(["consistent", "needs-review", "inconclusive"] as const)(
    "handles advisory %s research without turning findings into CRM mutations",
    async (status) => {
      const store = sqliteStore(":memory:");
      try {
        const demo = createDemoAttio();
        const originalOwner = structuredClone(
          demo.records.get("company_acme")!.values.account_owner,
        );
        ...
        ).assign(routingCases[0]!.input, { idempotencyKey: status });
        const worker = createWorkflowWorker({
          store: store.workflows,
          definition: createLeadWorkflow({
            research: async () => ({
              brief: "Evidence, not an executable change",
              sources: [],
              proposedChanges: [],
              findings: [
                { description: "Employee estimates differ; same territory band", sources: [] },
              ],
              review: { status, reason: "Policy consequence assessed" },
        ...
        await worker.runOnce();
        expect(store.workflows.get(decision.id)?.status).toBe(
          status === "consistent" ? "pending" : "awaiting_approval",
        );
        if (status !== "consistent")
          store.workflows.resolve(decision.id, {
            action: "keep-initial",
            actor: "reviewer",
            note: "Evidence reviewed; keep assignment",
          });
        await worker.runOnce();
        expect(store.workflows.get(decision.id)?.status).toBe("completed");
        expect(demo.records.get("company_acme")!.values.account_owner).toEqual(originalOwner);
        expect(demo.records.get("company_acme")!.values.routing_qualification).toBeUndefined();
        expect(demo.records.get("company_acme")!.values.routing_research).toEqual([
          { value: "Evidence, not an executable change" },
        ]);
      } finally {
        store.close();
      }
