...
Requires Docker, `OPENROUTER_API_KEY`, and `PARALLEL_API_KEY`. Root `.env.research.local` is loaded automatically. Optionally set `RESEARCH_MODEL`; the default is `openai/gpt-5.6-luna`. Build the sandbox from the repository root with `npm run research:build` first.
...
```sh
npm run start --workspace=@open-routing/example-research
```
